...
// Tool 5: Archive Thought
server.registerTool(
  "archive_thought",
  {
    title: "Archive Thought",
    description:
      "Archive a thought by its ID so it no longer appears in searches. Use this after completing a project, closing a loop, or when old context is no longer needed. Archived thoughts are not deleted — they can be unarchived later if needed.",
    inputSchema: {
      id: z.string().describe("The ID of the thought to archive"),
    },
  },
  async ({ id }) => {
    try {
...
// Tool 6: Unarchive Thought
server.registerTool(
  "unarchive_thought",
  {
    title: "Unarchive Thought",
    description:
      "Restore an archived thought so it appears in searches again.",
    inputSchema: {
      id: z.string().describe("The ID of the thought to unarchive"),
    },
  },
  async ({ id }) => {
    try {
      ...
      if (error) {
        return {
          content: [{ type: "text" as const, text: `Unarchive failed: ${error.message}` }],
          isError: true,
        };
      }
